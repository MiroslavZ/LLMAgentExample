"""Общие MCP-подключения и просмотр инструментов без обращения к модели."""
...
from ..mcp_config import GITHUB_MCP_URL, GITHUB_TOKEN_ENV, MCPServer, MCPServerStore, MCPStorageError
...
class MCPPanel:
    ...
    def _build_dialog(self) -> None:
        with self.root, ui.dialog() as self.dialog, ui.card().classes("mcp-dialog"):
            ...
            ui.label("Добавьте сервер и получите список его инструментов.").classes("memory-description")
            ...
            with ui.column().classes("mcp-editor") as self.editor:
                ...
                ui.label(
                    "Укажите имя переменной, например GITHUB_PERSONAL_ACCESS_TOKEN. "
                    "Сам токен добавьте в .env и перезапустите приложение. "
                    "Для сервера без авторизации оставьте поле пустым."
    ...
    def refresh(self, selected_id: str | None = None) -> None:
        try:
            ...
            self.address.set_text(selected.url if selected else "")
    ...
    def update_controls(self) -> None:
        ...
        for control in (self.selector, self.new_button, self.save_button,
                        self.name_input, self.url_input, self.token_input):
    ...
    def create(self) -> None:
        ...
        self.url_input.set_value(GITHUB_MCP_URL)
        self.token_input.set_value(GITHUB_TOKEN_ENV)
